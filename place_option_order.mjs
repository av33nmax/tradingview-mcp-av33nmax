...
// Per-trade risk cap. Aligned with trade_window.mjs (100 to support
// multi-trade-per-day mode; 2-3 trades × $100 = same total daily exposure
// as the original single-trade $300 cap).
const MAX_RISK_USD = 100;
...
if (!['SPY', 'QQQ', 'IWM', 'AAPL', 'NVDA', 'AMZN'].includes(ticker) || !['A', 'B'].includes(triggerArg)) {
  console.error('Usage: node place_option_order.mjs <SPY|QQQ|IWM|AAPL|NVDA|AMZN> <A|B>');
  process.exit(1);
}
...
async function main() {
  ...
  // ─── IBKR-side position guard ────────────────────────────────────────────
  // The hasTradedToday() check above only catches script-placed trades. A
  // manual TWS click bypasses it entirely. Anchor: 2026-05-04 SPY 722C ×2
  // manual entry placed AFTER the watcher's SPY trade had already used the
  // ticker's daily slot — −$42 loss bypassed the cap safely.
  // Defense: query IBKR for any existing OPT positions on this ticker.
  // If found, abort. Catches manual TWS positions, watcher fills, even
  // overnight orphan positions. Lenient on query failure (4s timeout).
  const existingPositions = await new Promise((resolve) => {
    const matches = [];
  ...
  if (existingPositions.length > 0) {
    console.error(`\n❌ ABORTING: ${existingPositions.length} existing OPT position(s) on ${ticker}:`);
    for (const p of existingPositions) {
      console.error(`   ${p.symbol} ${p.strike}${p.right} ${p.expiry} (${p.pos} contract${p.pos === 1 ? '' : 's'})`);
    }
    console.error(`   place_option_order.mjs refuses to place a second order while a position is open.`);
    console.error(`   If this is intentional (e.g. averaging in), close the existing position in TWS first.`);
    ib.disconnect();
    setTimeout(() => process.exit(1), 300);
    return;
  }
  console.log(`✅ no existing ${ticker} option positions — safe to proceed`);
